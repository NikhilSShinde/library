...
//        $orders = Order::select(array('order_unique_id', 'order_place_date_time', 'order_complete_date_time', 'cancelled_date', 'cancel_reson', 'fare_amount', 'waiting_charge', 'other_charges', 'country_id'))->get();
        ...
        if ($download == "excel") {
            Excel::create('Order Reports_' . date('d-M-Y'), function($excel)use($orders) {
                ...
                foreach ($orders as $order) {
                    ...
                    $arr_orders[$i]["fare_amount"] = $order->fare_amount;
                    $arr_orders[$i]["total_amount"] = $order->total_amount;
...
//                dd($arr_orders);
                ...
                $excel->sheet('Excel sheet', function($sheet)use($arr_orders) {
                    ...
                    $sheet->row(1, array(
                        'Order Number', 'Order Date', 'Posted Date', 'Customer Name', 'Delivery User Name', 'Service Name', 'Category Name', 'Estimated Amount', 'Total Amount', 'Payment Type', 'Status', 'Country', 'Order Type', 'Selected Pickup Latitude', 'Selected Pickup Longitude', 'Pickup Latitude', 'Pickup Longitude', 'Selected Drop Latitude', 'Selected Drop Longitude', 'Drop Latitude', 'Drop Longitude', 'Pickup Area', 'Drop Area', 'Contact Person For Pickup', 'Contact Person For Destination', 'Pickup Person Contact no', 'Destination Person Contact no', 'Distance', 'Item Description', 'Duration'
                    ));
...
//filter by user country
        ...
        if ($download == "excel") {
            ...
            Excel::create('Delivery User Reports_' . date('d-M-Y'), function($excel)use($star_users) {
                $arr_star_user = array();
                $i = 0;
                ...
                $excel->sheet('Excel sheet', function($sheet)use($arr_star_user) {
                    ...
                    $sheet->row(1, array(
                        'User Id', 'First Name', 'Last Name','Status', 'Mobile Number', 'Registration Date', 'Nationality/Country', 'State', 'City', 'Driver License File', 'Driver License', 'Geo Fence', 'Id Number', 'Availability Status', 'Profile Picture', 'Device Id', 'Device Type',
                    ));
            ...
            Excel::create('Delivery User Reports_' . date('d-M-Y'), function($excel)use($star_users) {
                ...
                $excel->sheet('Excel sheet', function($sheet)use($arr_star_user) {
                    ...
                    $sheet->row(1, array(
                        'User Id', 'First Name', 'Last Name','Status', 'Mobile Number','Registration Date', 'Country', 'State', 'City', 'Driver License File', 'Driver License', 'Geo Fence', 'Id Number', 'Availability Status', 'Profile Picture', 'Device Id', 'Device Type'
                    ));
